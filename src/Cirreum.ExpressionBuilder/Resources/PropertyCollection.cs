...
public class PropertyCollection : IPropertyCollection {
	...
	/// <summary>
	/// ResourceManager which the properties descriptions should be read from.
	/// </summary>
	...
	public Property? this[string propertyId] {
		get {
			return this.Properties.FirstOrDefault(p => p.Id.Equals(propertyId));
		}
	}
	...
	private static string GetPropertyResourceName(string propertyConventionName) {
	...
	private IEnumerable<Property> GetProperties(MemberInfo member) {
		...
		if (memberType.IsValueType || memberType == typeof(string)) {
			return [
				new Property(member.Name, member.Name, member)
			];
		}
	...
	public IList<Property> ToList() {
		var properties = new Property[this.Properties.Count];
